...
},
{
  ...
  "number": "1.3.6",
  "title": "",
  "body": "   Let be defined by for some (i.e. is a half-space of ).      Let . Explain why for any .      Show that for any , that .      Let , that is, there are such that and . Show that for .      Conclude that is convex.    "
},
{
...
},
{
  ...
  "number": "2.3.6",
  "title": "",
  "body": "  If we follow , then no basis is visited twice during the Simplex Algorithm. Note that is nondecreasing with each pivot. Must the Simplex Algorithm terminate? Why?   "
},
{
  "id": "sec-simplexsage",
  "level": "1",
...
},
{
  ...
  "number": "6.2.2",
  "title": "",
  "body": "  Recall the transportation tableau obtained in :        NA   NA   NA       NA                 NA                 NA                                Note that this tableau corresponds to a basic feasible solution for the original minimization problem.     If this solution is not optimal, what should the next step be?    Pick a variable to exit the basis which increases the objective function and make the smallest change.    Pick a variable to exit the basis which decreases the objective function and make the smallest change.    Pick a variable to exit the basis which increases the objective function and make the biggest change.    Pick a variable to exit the basis which decreases the objective function and make the biggest change.        Select row values and column values so that each circled value is the sum of row and column values .  (We can think of these as analogous to the shadow costs associated with the warehouse\/market bounds.)                                                                   Replace each entry of the tableau with . What does this measure? What does it mean if each entry is nonnegative?      Pick a .                                                                   Pick circled entries so that they with the boxed form a cycle , that is each of these entries shares a row with exactly one another of the entries, and a column with another of the entries.      Based on the discussion in , which entry should transfer their shipments to ?     .     .     .     .     .    How do the other entries in the cycle adjust? (There may be more than one valid choice.)      Remove the basis entry which is no longer being used, and recompute the with the new basis.                                                                                     Verify that none of the entries are non negative. Why do we now terminate?  We then replace the entries with the original entries:                                                                   Use Sage to confirm the solution:        "
},
{
...
},
{
  ...
  "number": "7.5.3",
  "title": "",
  "body": "  Consider the max-flow min-cut problem  .     Write out the non-canonical maximization problem which would compute the max-flow.      Record this problem in a Tucker tableau, then record the dual variable using to denote dual variables associated with vertex equality constraints and to denote the dual variables for edge inequality constraint.     Verify that the max-flow and min-cut you found are feasible solutions to these problems (using the convention that if , if and if and otherwise.)     Argue that any cut corresponds to a feasible solution to this dual problem.      How can we tell both the flow and cut found in  are optimal? (Think duality.)    "
},
{
  "id": "exercises-flow-4",
  "level": "2",
